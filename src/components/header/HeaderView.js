    ...
    return (
        <header className='flex items-center justify-between w-full px-6 py-4 h-fit md:col-start-1 md:col-span-full md:px-4 md-lg:px-0 md-lg:col-start-2 md-lg:col-span-9 md-lg:min-w-fit lg:col-start-3 lg:col-span-8 md:border-b md:border-b-gray-blue z-40'>
            <div className='flex items-center justify-between w-50 md:w-auto'>
                ...
                <h1 className='ml-4 text-3xl font-bold font-primary cursor-pointer text-vark-blue md:ml-0 md:mr-10 md-lg:text-4xl'>
            ...
            <div className={`fixed left-0 top-0 transition-all duration-700 ease-in-out ${model.openMenuModel ? 'bg-black z-[70] visible' : 'bg-transparent invisible'} w-full h-full md:contents`}>
                <nav className={`absolute left-0 top-0 h-full transition-all duration-300 ease-in ${model.openMenuModel ? 'w-2/3 pl-4' : 'w-0 pl-0'} pt-20 ease-in-out ${model.menuDisplay}  bg-white dark:bg-black text-vark-blue md:pl-0 md:visible md:relative md:pt-0 md:h-auto md:w-auto  md:flex md:items-center md:text-neutral-900`}>
                        ...
                        {Children.toArray(
                            model.menuLinks.map(linkPages => (
                                <li onClick={model.handleNavLinkActive} className='
					            		...
					            		md:text-dark-gray-blue md:font-normal md:ml-6 
					            		md:first:ml-0 md:transition-all hover:text-blue-500 

						            	md:after:content-[""] md:after:absolute
						            	md:after:left-0 md:after:top-12 
						            	md:after:rounded-sm 
						            	md:after:w-full md:after:h-1/5 
