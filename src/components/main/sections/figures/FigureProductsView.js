...
const FigureProductsView = ({ handleGetBtnSlide, slideProduct }) => {
	return (
		<div className={` relative w-full h-full md:row-start-1 md:row-span-2`}>
				...
				<ul className='h-full'>
					{Children.toArray(
						PRODUCTS.map((product, index) => {
							return (
								<li>
									<img ref={element => slideProduct.current[index] = element} className={`absolute left-0 top-0 object-cover h-full w-full transition-opacity duration-500 ${index === 0 ? 'opacity-100' : 'opacity-0'} md:rounded-2xl cursor-pointer z-0`}
			...
			<button id='prev-slide' onClick={handleGetBtnSlide} className={`flex justify-center items-center absolute inset-y-0 my-auto border-2 bg-white h-10 ml-4 w-10 sm:w-12 sm:h-12 sm-md:w-14 sm-md:h-14 rounded-full text-sm `}>
